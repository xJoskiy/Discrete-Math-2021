...
def ADD_PP_P(x, xs, y, ys):
    ...
    while i != lenght:
        if (xs[i] > ys[i]) and (not (len(y) < len(ys))):
            ...
            res.append(y[i])
        elif (xs[i] == ys[i]) and (xs[i] != 0) and (ys != 0):
            print("Это значение передается в функцию ADD_QQ_Q  x^", xs[i], " = ", x[i])
            print("Это значение передается в функцию ADD_QQ_Q  y^", ys[i], " = ", y[i])
